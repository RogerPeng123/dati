...
Eof
     ...
     * @return array
     */
    public function update_admin_name_password($attributes, $id)
    {
        // 修改密码
        if ($attributes['password']) {
            $attributes['password'] = bcrypt($attributes['password']);
        ...
        $result = User::where(['id' => Auth::id()])->update($attributes);

        if ($result) {
            return ['status' => 200, 'message' => '更新成功', 'data' => []];
        } else {
            return ['status' => 401, 'message' => '更新失败', 'data' => []];
        }
    }
