    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-2">
            <div class="title-action">
                @if(haspermission('newstypecontroller.create'))
                    <a href="{{ route('news-type.create') }}" class="btn btn-info">
                        <i class="fa fa-plus"></i> {!! trans('common.create')!!} 类别
                    </a>
                @endif
            </div>
        </div>
                                        ...
                                        @if($data->total())
                                            @foreach($data as $item)
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td>
                                                        <a href="{{ route('news-type.edit',['id'=>encodeId($item->id)]) }}"
                                                           class="btn btn-xs btn-outline btn-warning tooltips">
                                                            <i class="fa fa-edit"></i>
                                                        </a>

                                                        <a href="javascript:;"
                                                           destroy-url="{{ route('news-type.destroy',['id'=>encodeId($item->id)]) }}"
...
@section('js')
    ...
    <script type="text/javascript">
        $(document).on('click', '.destroy_item', function () {
            var _item = $(this);
            var title = "{{trans('common.deleteTitle')}}新闻类别？";
            layer.confirm(title, {
